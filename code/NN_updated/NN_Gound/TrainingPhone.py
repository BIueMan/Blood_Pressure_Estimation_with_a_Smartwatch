...
def train(images, doTrain = True):
    ...
    for key, image in images.items():
        ...
        # load data to GPU
        image, label = Variable(image.to(device=device, dtype=torch.float)), Variable(label.to(device=device, dtype=torch.float))
...
""" get mean of label """
sum_labels = 0
for key, image in images_train.items():
    label = key.split("_")[-1]
    label = float(label)
    sum_labels += label

mean_labels = sum_labels/len(images_train)
print("mean label is: ", mean_labels)
...
for epoch in range(num_epoch):  # loop over the dataset multiple times
    ...
    print("test: " + " loss: " + str(mean_loss_test) + ", accuracy: " + str(accuracy_test) + ", dist: " + str(mean_dist_test))
...
plt.subplot(313)
plt.plot(t, train_dist)
plt.plot(t, test_dist)
plt.xlabel('epoch')
plt.ylabel('distance')
